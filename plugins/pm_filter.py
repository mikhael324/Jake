...
@Client.on_callback_query(filters.regex(r"^sang"))
async def season_check(bot, query):
    _, userid, Season = query.data.split("#")
...
async def select_season (bot, query):
    ...
    if int(userid) not in [query.from_user.id, 0]:
        return await query.answer(script.ALRT_TXT.format(query.from_user.first_name), show_alert=True)
    btn = [[
        InlineKeyboardButton("Sᴇʟᴇᴄᴛ Yᴏᴜʀ Dᴇꜱɪʀᴇᴅ sᴇᴀsᴏɴ", callback_data=f"sang#{userid}#unknown")
    ],[
        InlineKeyboardButton("Season 1", callback_data=f"sang#{userid}#s01"),       
        InlineKeyboardButton("Season 2", callback_data=f"sang#{userid}#s02")
    ],[
        InlineKeyboardButton("Season 3", callback_data=f"sang#{userid}#s03"),
        InlineKeyboardButton("Season 4", callback_data=f"sang#{userid}#s04")
    ],[
        InlineKeyboardButton("Season 5", callback_data=f"sang#{userid}#s05"),
        InlineKeyboardButton("Season 6", callback_data=f"sang#{userid}#s06")
    ],[
    ...
    if query.data.startswith("file"):
                    ...
                    if IS_VERIFY and not await check_verification(client, query.from_user.id):
                        ...
                        await client.send_cached_media(
                            ...
                            reply_markup=InlineKeyboardMarkup(
                                [
                                [
                                InlineKeyboardButton('Oᴛᴛ Uᴩᴅᴀᴛᴇꜱ', url="https://t.me/+p2eDKBfXPLk3ZGY1")
                            ],[
        ...
        await query.message.edit_text(
            ...
            btn = [[
                    ...
                    InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="t.me/Amalchristo")
                  ]]
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
